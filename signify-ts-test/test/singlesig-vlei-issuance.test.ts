...
beforeAll(async () => {
  ...
  assert(
    env.secrets.length == 4,
    "Expected 4 secrets in order to cover the 4 roles",
  );
});
...
test("singlesig-vlei-issuance", async function run() {
  ...
  if (!ecrAuthCredHolder) {
    ...
    ecrAuthCredHolder = await retry(async () => {
      const cred = await getReceivedCredential(qviClient, ecrAuthCred.sad.d);
      assert(cred !== undefined);
      return cred;
    }, CRED_RETRY_DEFAULTS);
  }
  ...
  if (!oorAuthCredHolder) {
    ...
    oorAuthCredHolder = await retry(async () => {
      const cred = await getReceivedCredential(qviClient, oorAuthCred.sad.d);
      assert(cred !== undefined);
      return cred;
    }, CRED_RETRY_DEFAULTS);
  }
