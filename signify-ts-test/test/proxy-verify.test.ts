...
test("reg-pilot-proxy", async function run() {
  // try to ping the api
  let ppath = "/ping";
  let preq = { method: "GET", body: null };
  let presp = await fetch(env.proxyBaseUrl + ppath, preq);
  ...
  let lpath = `/login`;
  let lresp = await fetch(env.proxyBaseUrl + lpath, lreq);
  ...
  let cresp = await fetch(env.proxyBaseUrl + cpath, creq);
  ...
  sresp = await fetch(env.proxyBaseUrl + spath, sreq);
...
async function getReportStatusByAid(
  ...
  const surl = `${env.proxyBaseUrl}/status/${aidPrefix}`;
...
async function getReportStatusByDig(
  ...
  const heads = new Headers();
  const sreq = { headers: heads, method: "GET", body: null };
  const surl = `${env.proxyBaseUrl}/upload/${aidPrefix}/${dig}`;
  let shreq = await client.createSignedRequest(aidName, surl, sreq);
  const sresp = await fetch(surl, shreq);
  return sresp;
}
...
async function dropReportStatusByAid(aidPrefix: string): Promise<Response> {
  ...
  const surl = `${env.proxyBaseUrl}/status/${aidPrefix}/drop`;
...
async function uploadReport(
  ...
  const url = `${env.proxyBaseUrl}/upload/${aidPrefix}/${zipDigest}`;
