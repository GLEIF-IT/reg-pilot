...
const { vleiServerUrl, witnessIds, lei, secrets, roleName } =
  resolveEnvironment();
...
test("multisig-vlei-issuance", async function run() {
  ...
  const [
    ...
    clientLAR3,
    clientECR,
  ] = await getOrCreateClients(secrets.length, secrets, false);
  ...
  const [
    ...
    getOrCreateAID(clientECR, roleName, kargsAID),
  ]);
  ...
  const [
    ...
    clientECR.oobis().get(roleName, "agent"),
  ]);
  ...
  await Promise.all([
    ...
    getOrCreateContact(clientLAR1, roleName, oobiECR.oobis[0]),
    getOrCreateContact(clientLAR2, roleName, oobiECR.oobis[0]),
    getOrCreateContact(clientLAR3, roleName, oobiECR.oobis[0]),
  ]);
  ...
  try {
    ...
    const queryOp1 = await clientQAR1.keyStates().query(aidGEDA.prefix, "1");
    const queryOp2 = await clientQAR2.keyStates().query(aidGEDA.prefix, "1");
    const queryOp3 = await clientQAR3.keyStates().query(aidGEDA.prefix, "1");
  ...
  leRegistrybyLAR1 = leRegistrybyLAR1.filter(
    (reg: { name: string }) => reg.name == `leRegistry${aidECR.prefix}`,
  );
