...
export class VleiIssuance {
    ...
    constructor(secretsJsonFile: string) {
        this.secretsJson = JSON.parse(fs.readFileSync(this.secretsJsonPath + secretsJsonFile, 'utf-8'));
    }
    ...
    public async prepareClients() {
        ...
        await this.resolveOobis([QVI_SCHEMA_URL,
            LE_SCHEMA_URL,
            ECR_AUTH_SCHEMA_URL,
            ECR_SCHEMA_URL,
            OOR_AUTH_SCHEMA_URL,
                ...
                for (let i = 0; i < clientListA.length; i++) {
                    for (const [roleB, clientListB] of user.clients) {
                        for (let j = 0; j < clientListB.length; j++) {
                            if (roleA !== roleB || i !== j) { // Skip self-referencing
                                contactPromises.push(
                                    getOrCreateContact(clientListA[i], `${roleB}${j + 1}`, user.oobis.get(roleB)?.[j].oobis[0])
                                );
                            }
                        }
                    }
    ...
    public async issueCredentials() {
        for (const user of this.users) {
            if (user.clients.get("gleif")!.length > 1 || user.clients.get("qvi")!.length > 1 || user.clients.get("le")!.length > 1) {
    ...
    private async mutiSigVleiIssuance(user: User) {
        ...
        try {
            aidGEDAbyGAR1 = await user.clients.get("gleif")![0].identifiers().get("GEDA");
            aidGEDAbyGAR2 = await user.clients.get("gleif")![1].identifiers().get("GEDA");
        } catch {
            const rstates = [user.aids.get("gleif")![0].state, user.aids.get("gleif")![1].state];
            ...
            await waitAndMarkNotification(user.clients.get("gleif")![0], "/multisig/icp");
        ...
        if (oobiGEDAbyGAR1.oobis.length == 0 || oobiGEDAbyGAR2.oobis.length == 0) {
            ...
            await Promise.all(opList1.map((op: any) => waitOperation(user.clients.get("gleif")![0], op)));
            await Promise.all(opList2.map((op: any) => waitOperation(user.clients.get("gleif")![1], op)));

            await waitAndMarkNotification(user.clients.get("gleif")![0], "/multisig/rpy");
        ...
        try {
            aidQVIbyQAR1 = await user.clients.get("qvi")![0].identifiers().get(QVI_INTERNAL_NAME);
            aidQVIbyQAR2 = await user.clients.get("qvi")![1].identifiers().get(QVI_INTERNAL_NAME);
            aidQVIbyQAR3 = await user.clients.get("qvi")![2].identifiers().get(QVI_INTERNAL_NAME);
        } catch {
            const rstates = [user.aids.get("qvi")![0].state, user.aids.get("qvi")![1].state, user.aids.get("qvi")![2].state];
            const states = rstates;
            ...
            await waitAndMarkNotification(user.clients.get("gleif")![0], "/multisig/ixn");

            // QARs query the GEDA's key state
            const queryOp1 = await user.clients.get("qvi")![0].keyStates().query(aidGEDA.prefix, "1");
            const queryOp2 = await user.clients.get("qvi")![1].keyStates().query(aidGEDA.prefix, "1");
            const queryOp3 = await user.clients.get("qvi")![2].keyStates().query(aidGEDA.prefix, "1");
            ...
            await waitAndMarkNotification(user.clients.get("qvi")![0], "/multisig/icp");
        ...
        if (
            ...
            await Promise.all(opList1.map((op: any) => waitOperation(user.clients.get("qvi")![0], op)));
            await Promise.all(opList2.map((op: any) => waitOperation(user.clients.get("qvi")![1], op)));
            await Promise.all(opList3.map((op: any) => waitOperation(user.clients.get("qvi")![2], op)));

            await waitAndMarkNotification(user.clients.get("qvi")![0], "/multisig/rpy");
            await waitAndMarkNotification(user.clients.get("qvi")![1], "/multisig/rpy");
        ...
        if (gedaRegistrybyGAR1.length == 0 && gedaRegistrybyGAR2.length == 0) {
            ...
            await waitAndMarkNotification(user.clients.get("gleif")![0], "/multisig/vcp");
        ...
        if (!(qviCredbyGAR1 && qviCredbyGAR2)) {
            ...
            await waitAndMarkNotification(user.clients.get("gleif")![0], "/multisig/iss");
            ...
            await waitAndMarkNotification(user.clients.get("gleif")![0], "/multisig/exn");
        }
        ...
        let qviCredbyQAR1 = await getReceivedCredential(user.clients.get("qvi")![0], qviCred.sad.d);
        let qviCredbyQAR2 = await getReceivedCredential(user.clients.get("qvi")![1], qviCred.sad.d);
        let qviCredbyQAR3 = await getReceivedCredential(user.clients.get("qvi")![2], qviCred.sad.d);
        if (!(qviCredbyQAR1 && qviCredbyQAR2 && qviCredbyQAR3)) {
            ...
            await waitAndMarkNotification(user.clients.get("gleif")![0], "/exn/ipex/admit");
            await waitAndMarkNotification(user.clients.get("gleif")![1], "/exn/ipex/admit");
            await waitAndMarkNotification(user.clients.get("qvi")![0], "/multisig/exn");
            await waitAndMarkNotification(user.clients.get("qvi")![1], "/multisig/exn");
            await waitAndMarkNotification(user.clients.get("qvi")![2], "/multisig/exn");
            await waitAndMarkNotification(user.clients.get("qvi")![0], "/exn/ipex/admit");
            await waitAndMarkNotification(user.clients.get("qvi")![1], "/exn/ipex/admit");
            await waitAndMarkNotification(user.clients.get("qvi")![2], "/exn/ipex/admit");

            qviCredbyQAR1 = await waitForCredential(user.clients.get("qvi")![0], qviCred.sad.d);
            qviCredbyQAR2 = await waitForCredential(user.clients.get("qvi")![1], qviCred.sad.d);
            qviCredbyQAR3 = await waitForCredential(user.clients.get("qvi")![2], qviCred.sad.d);
        }
        ...
        try {
            aidLEbyLAR1 = await user.clients.get("le")![0].identifiers().get(LE_INTERNAL_NAME);
            aidLEbyLAR2 = await user.clients.get("le")![1].identifiers().get(LE_INTERNAL_NAME);
            aidLEbyLAR3 = await user.clients.get("le")![2].identifiers().get(LE_INTERNAL_NAME);
        } catch {
            const rstates = [user.aids.get("le")![0].state, user.aids.get("le")![1].state, user.aids.get("le")![2].state];
            const states = rstates;
            ...
            await waitAndMarkNotification(user.clients.get("le")![0], "/multisig/icp");
        ...
        if (
            ...
            await Promise.all(opList1.map((op: any) => waitOperation(user.clients.get("le")![0], op)));
            await Promise.all(opList2.map((op: any) => waitOperation(user.clients.get("le")![1], op)));
            await Promise.all(opList3.map((op: any) => waitOperation(user.clients.get("le")![2], op)));

            await waitAndMarkNotification(user.clients.get("le")![0], "/multisig/rpy");
            await waitAndMarkNotification(user.clients.get("le")![1], "/multisig/rpy");
        ...
        if (
            ...
            await waitAndMarkNotification(user.clients.get("qvi")![0], "/multisig/vcp");
        ...
        if (!(leCredbyQAR1 && leCredbyQAR2 && leCredbyQAR3)) {
            ...
            await waitAndMarkNotification(user.clients.get("qvi")![0], "/multisig/iss");
        ...
        let leCredbyLAR1 = await getReceivedCredential(user.clients.get("le")![0], leCred.sad.d);
        let leCredbyLAR2 = await getReceivedCredential(user.clients.get("le")![1], leCred.sad.d);
        let leCredbyLAR3 = await getReceivedCredential(user.clients.get("le")![2], leCred.sad.d);
        if (!(leCredbyLAR1 && leCredbyLAR2 && leCredbyLAR3)) {
            ...
            await waitAndMarkNotification(user.clients.get("qvi")![1], "/exn/ipex/admit");
            await waitAndMarkNotification(user.clients.get("qvi")![2], "/exn/ipex/admit");
            await waitAndMarkNotification(user.clients.get("le")![0], "/multisig/exn");
            await waitAndMarkNotification(user.clients.get("le")![1], "/multisig/exn");
            await waitAndMarkNotification(user.clients.get("le")![2], "/multisig/exn");
            await waitAndMarkNotification(user.clients.get("le")![0], "/exn/ipex/admit");
            await waitAndMarkNotification(user.clients.get("le")![1], "/exn/ipex/admit");
            await waitAndMarkNotification(user.clients.get("le")![2], "/exn/ipex/admit");

            leCredbyLAR1 = await waitForCredential(user.clients.get("le")![0], leCred.sad.d);
            leCredbyLAR2 = await waitForCredential(user.clients.get("le")![1], leCred.sad.d);
            leCredbyLAR3 = await waitForCredential(user.clients.get("le")![2], leCred.sad.d);
        }
        ...
        leRegistrybyLAR1 = leRegistrybyLAR1.filter(
            (reg: { name: string }) => reg.name == `leRegistry${user.aids.get("ecr")![0].prefix}`,
        );
        ...
        if (
            ...
            await waitAndMarkNotification(user.clients.get("le")![0], "/multisig/vcp");

            [leRegistrybyLAR1, leRegistrybyLAR2, leRegistrybyLAR3] = await Promise.all([
                user.clients.get("le")![0].registries().list(aidLE.name),
                user.clients.get("le")![1].registries().list(aidLE.name),
                user.clients.get("le")![2].registries().list(aidLE.name),
            ]);
        ...
        if (!(ecrCredbyLAR1 && ecrCredbyLAR2 && ecrCredbyLAR3)) {
            ...
            await waitAndMarkNotification(user.clients.get("le")![0], "/multisig/iss");
        ...
        console.log("LE has issued an ECR vLEI credential with SAID:", ecrCred.sad.d);

        // LE and ECR Person exchange grant and admit messages.
        // Skip if ECR Person has already received the credential.
        let ecrCredbyECR = await getReceivedCredential(user.clients.get("ecr")![0], ecrCred.sad.d);
        if (!ecrCredbyECR) {
            await admitSinglesig(user.clients.get("ecr")![0], user.aids.get("ecr")![0].name, aidLE);
            await waitAndMarkNotification(user.clients.get("le")![0], "/exn/ipex/admit");
            await waitAndMarkNotification(user.clients.get("le")![1], "/exn/ipex/admit");
            await waitAndMarkNotification(user.clients.get("le")![2], "/exn/ipex/admit");

            ecrCredbyECR = await waitForCredential(user.clients.get("ecr")![0], ecrCred.sad.d);
        }
    ...
    private async singleSigVleiIssuance(user: User) {
        ...
        const [gleifRegistry, qviRegistry, leRegistry] = await Promise.all([
            getOrCreateRegistry(user.clients.get("gleif")![0], user.aids.get("gleif")![0], "gleifRegistry"),
            getOrCreateRegistry(user.clients.get("qvi")![0], user.aids.get("qvi")![0], "qviRegistry"),
            getOrCreateRegistry(user.clients.get("le")![0], user.aids.get("le")![0], `leRegistry${user.aids.get("ecr")![0].prefix}`),
        ]);
        ...
        let qviCredHolder = await getReceivedCredential(user.clients.get("qvi")![0], qviCred.sad.d);

        if (!qviCredHolder) {
            await sendGrantMessage(user.clients.get("gleif")![0], user.aids.get("gleif")![0], user.aids.get("qvi")![0], qviCred);
            await sendAdmitMessage(user.clients.get("qvi")![0], user.aids.get("qvi")![0], user.aids.get("gleif")![0]);

            qviCredHolder = await retry(async () => {
                const cred = await getReceivedCredential(user.clients.get("qvi")![0], qviCred.sad.d);
        ...
        let leCredHolder = await getReceivedCredential(user.clients.get("le")![0], leCred.sad.d);

        if (!leCredHolder) {
            await sendGrantMessage(user.clients.get("qvi")![0], user.aids.get("qvi")![0], user.aids.get("le")![0], leCred);
            await sendAdmitMessage(user.clients.get("le")![0], user.aids.get("le")![0], user.aids.get("qvi")![0]);

            leCredHolder = await retry(async () => {
                const cred = await getReceivedCredential(user.clients.get("le")![0], leCred.sad.d);
                assert(cred !== undefined);
                return cred;
            }, CRED_RETRY_DEFAULTS);
        }
        ...
        let ecrCredHolder = await getReceivedCredential(user.clients.get("ecr")![0], ecrCred.sad.d);

        if (!ecrCredHolder) {
            await sendGrantMessage(user.clients.get("le")![0], user.aids.get("le")![0], user.aids.get("ecr")![0], ecrCred);
            await sendAdmitMessage(user.clients.get("ecr")![0], user.aids.get("ecr")![0], user.aids.get("le")![0]);

            ecrCredHolder = await retry(async () => {
                const cred = await getReceivedCredential(user.clients.get("ecr")![0], ecrCred.sad.d);
                assert(cred !== undefined);
                return cred;
            }, CRED_RETRY_DEFAULTS);
        }
        ...
        const roleAidPrefix = user.aids.get("qvi")![0].prefix + user.aids.get("ecr")![0].prefix;
        ...
        if (!ecrAuthCredHolder) {
            await sendGrantMessage(user.clients.get("le")![0], user.aids.get("le")![0], user.aids.get("qvi")![0], ecrAuthCred);
            await sendAdmitMessage(user.clients.get("qvi")![0], user.aids.get("qvi")![0], user.aids.get("le")![0]);

            ecrAuthCredHolder = await retry(async () => {
                const cred = await getReceivedCredential(user.clients.get("qvi")![0], ecrAuthCred.sad.d);
                assert(cred !== undefined);
                return cred;
            }, CRED_RETRY_DEFAULTS);
        }
        ...
        let ecrCredHolder2 = await getReceivedCredential(user.clients.get("ecr")![0], ecrCred2.sad.d);

        if (!ecrCredHolder2) {
            await sendGrantMessage(user.clients.get("qvi")![0], user.aids.get("qvi")![0], user.aids.get("ecr")![0], ecrCred2);
            await sendAdmitMessage(user.clients.get("ecr")![0], user.aids.get("ecr")![0], user.aids.get("qvi")![0]);

            ecrCredHolder2 = await retry(async () => {
                const cred = await getReceivedCredential(user.clients.get("ecr")![0], ecrCred2.sad.d);
                assert(cred !== undefined);
                return cred;
            }, CRED_RETRY_DEFAULTS);
        }
        ...
        if (!oorAuthCredHolder) {
            await sendGrantMessage(user.clients.get("le")![0], user.aids.get("le")![0], user.aids.get("qvi")![0], oorAuthCred);
            await sendAdmitMessage(user.clients.get("qvi")![0], user.aids.get("qvi")![0], user.aids.get("le")![0]);

            oorAuthCredHolder = await retry(async () => {
                const cred = await getReceivedCredential(user.clients.get("qvi")![0], oorAuthCred.sad.d);
                assert(cred !== undefined);
                return cred;
            }, CRED_RETRY_DEFAULTS);
        }
        ...
        let oorCredHolder = await getReceivedCredential(user.clients.get("ecr")![0], oorCred.sad.d);

        if (!oorCredHolder) {
            await sendGrantMessage(user.clients.get("qvi")![0], user.aids.get("qvi")![0], user.aids.get("ecr")![0], oorCred);
            await sendAdmitMessage(user.clients.get("ecr")![0], user.aids.get("ecr")![0], user.aids.get("qvi")![0]);

            oorCredHolder = await retry(async () => {
                const cred = await getReceivedCredential(user.clients.get("ecr")![0], oorCred.sad.d);
                assert(cred !== undefined);
                return cred;
            }, CRED_RETRY_DEFAULTS);
        }
        ...
        await assertOperations(user.clients.get("gleif")![0], user.clients.get("qvi")![0], user.clients.get("le")![0], user.clients.get("ecr")![0]);
        await warnNotifications(user.clients.get("gleif")![0], user.clients.get("qvi")![0], user.clients.get("le")![0], user.clients.get("ecr")![0]);
    }
}
