...
import { generate_reports, getEbaSignedReport } from "./report";
import {
  ApiUser,
  getApiTestData,
  getConfig,
...
} from "../../test/reg-pilot-api";
import { run_vlei_verification_test } from "../../test/vlei-verification";

import fs from "fs";
import yaml from "js-yaml";
import { createZipWithCopies } from "./bank-reports";
...
export async function runWorkflow(
  ...
  for (const [k, v] of Object.entries(workflow.workflow.steps)) {
    console.log(`Executing step: ${k} with config: ${JSON.stringify(v)}`);
  ...
  async function executeStep(
    ...
    if (step.type == "issue_credential") {
      ...
      if (step.test_case == "api_test_revocation") {
        ...
        await run_api_admin_test(apiUsers, adminUser[0]);
      } else if (step.test_case == "api_test_no_delegation") {
        await run_api_test_no_delegation(apiUsers);
