...
const options = {
  ...
  onProxyReq: (proxyReq: ClientRequest, req: Request, res: Response) => {
    ...
    console.log(`Proxy server received a request for ${req.url} and got headers ${JSON.stringify(req.headers)} and is sending a proxy request to ${API_SERVICE_URL} with headers ${JSON.stringify(proxyReq.getHeaders())}`);
  }
};
